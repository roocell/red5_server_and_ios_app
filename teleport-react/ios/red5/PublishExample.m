...
-(void)viewDidAppear:(BOOL)animated{
    ...
    [dict addEntriesFromDictionary:[NSMutableDictionary
                                    ...
                                    self.stream_server_ip,@"domain",
                                    @"live", @"context",
                                    self.stream_server_port, @"port",
  ...
  TGLog(@"trying %@ %d", config.host, config.port);
    ...
    [self.publish publish:_stream_identifier type:R5RecordTypeLive];

  
  
  UIButton *but= [UIButton buttonWithType:UIButtonTypeRoundedRect];
  [but addTarget:self action:@selector(buttonClicked:) forControlEvents:UIControlEventTouchUpInside];
  [but setFrame:CGRectMake(10, 32, 64, 64)];
